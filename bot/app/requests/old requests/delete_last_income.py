...
async def delete_last_income(telegram_id):
    load_dotenv()
    ...
    last_post = await get_last_income(telegram_id=telegram_id)
    if not last_post:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.delete(
            base_url + f"api/incomes/{last_post.get('id')}/", 
            headers=headers,
        ) as response:
...
async def main():
    response_data = await delete_last_income(telegram_id="6911237041")
    if response_data:
